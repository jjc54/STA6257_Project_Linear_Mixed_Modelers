...
Mean Sqaured Error (MSE) and Mean Absolute Error (MAE) are metrics asses the performance of a model. MSE is the mean of the individual residuals squared and MAE is the mean of the individual absolute value of the residuals. As shown in the grpahs below, the Imputed Final Model out performs the other two models by a signifciant margin. It is impotant to note that MSE is harsher to larger errors or outliers becuase sqaures the residuals
...
The bar raph below compares the Actual R5Hz_PP of 10 random subjects. The difference in the bars for each subject is the residual error.
