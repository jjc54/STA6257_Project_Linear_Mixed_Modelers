...
$$
\begin{equation}Y_{ij} = (\beta_0 + u_{0j}) + \beta_1(\text{BMI}_{ij}) + \beta_2(\text{AsthmaYes}_{ij}) + \beta_3(\text{ICSYes}_{ij}) + \beta_4(\text{LABA}_{ij}) + \beta_5(\text{Gender}_{ij}) + \beta_6(\text{Age_months}_{ij}) + \beta_7(\text{Height_cm}_{ij}) + \beta_8(\text{Weight_Kg}_{ij}) + \epsilon_{ij}\end{equation}
$$\
...
$$
\begin{align*}Y_{ijk} = & (\beta_0 + u_{0j} + u_{1j} \times \text{Observation_number2}_{ijk} + \dots + u_{5j} \times \text{Observation\_number6}_{ijk}) \\& + \beta_1(\text{BMI}_{ijk}) + \beta_2(\text{AsthmaYes}_{ijk}) + \beta_3(\text{ICSYes}_{ijk}) + \beta_4(\text{LABA}_{ijk}) \\& + \beta_5(\text{Gender}_{ijk}) + \beta_6(\text{Age_months}_{ijk}) + \beta_7(\text{Height_cm}_{ijk}) + \beta_8(\text{Weight_Kg}_{ijk}) \\& + \beta_9(\text{Group}_{ijk}) + \epsilon_{ijk}\end{align*}
$$
...
Mean Squared Error (MSE) and Mean Absolute Error (MAE) are metrics asses the performance of a model. MSE is the mean of the individual residuals squared and MAE is the mean of the individual absolute value of the residuals. As shown in the graphs below, the Imputed Final Model out performs the other two models by a signifciant margin. It is important to note that MSE is harsher to larger errors or outliers because it squares the residuals
...
The bar graph below compares the Actual R5Hz_PP to predicted R5Hz_PP of 10 random subjects. The difference in the bars for each subject is the residual error.
...
ggplot(sample_pred_actuals_melt, aes(x = factor(row), y = value, fill = variable)) +
  ...
  ggtitle("Fig 24. Sample Comparison of Predicted and Actual Values")
