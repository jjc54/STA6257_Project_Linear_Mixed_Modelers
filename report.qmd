...
The count plots show frequency distributions for categroies within a distribution. This can aid in data cleaning, checking sparseness or checking class imbalances. For examples in the count plots below we see that Asthma and LABA variables are binary varibales that are imbalances towards the "Yes" category
...
This shows the frequency and distributions of numerical variables This helps idenitfy distributions types amonngst the different variables. Most of the variables below exhibit a normal distribution with some (BMI) showing a slight right skew.
...
This plot is a visual aid to asses normality in the covariates. The closer the points are to the straight diagonal line the more "normal" the data is distributed. Most of varaibles shows a normal distribution. BMI has a substantial amount of points on the right corner of the plot that go off of the diagonal line and is due to the right skew in the histogram we mentioned in the plot sumary above.

```{r}
plot_qq(na.omit(x))
```

**Princicap Component Analysis**

The plot below shows the numerical variables in our data set split into principal components. More than have of the explained variance can be accounted for with just 4 principal componenets. This can be useful if we want to simplify our model by only keeping the principal components that explain most of the variance.
...
**It will be important to account for the rapid participant dropout that seems to be present in this dataset. It may or may not be attributed to the condition/treatment and thus may or may not be random. If not, it should be accounted for in the model.**

**Per Dr. Cohen 19MAR2024** - can still do LMM even with dropout. Imputation for missing datapoints is an option, but a strength of LMM is that it can handle unbalanced groups (patients). Most reviewers don't like extreme imputation in this case. Just monitor trend in dropouts due to potential link back to the treatment/study. If none, just continue with modeling.
